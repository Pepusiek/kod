...
from angles import deg2grad
from angles import grad2deg
from angles import grad2rad
from angles import rad2grad
from angles import decimal_deg2rad
from angles import rad2decimal_deg
from angles import decimal_deg2dms_deg
from angles import dms_deg2decimal_deg
...
def test_grad2rad():
    assert grad2rad(0) == 0
    assert grad2rad(50) == 0.7853981633974483
    assert grad2rad(100) == 1.5707963267948966
    assert grad2rad(150) == 2.356194490192345
    assert grad2rad(200) == 3.141592653589793
    assert grad2rad(250) == 3.926990816987241
    assert grad2rad(300) == 4.71238898038469
    assert grad2rad(350) == 5.497787143782137
    assert grad2rad(400) == 6.283185307179586
...
def test_dms_deg2decimal_deg():
    assert dms_deg2decimal_deg([252, 27, 20]) == 252.45555555555555
    assert dms_deg2decimal_deg([3, 2, 1]) == 3.033611111111111


if __name__ == '__main__':
   test_decimal_deg2dms_deg()
   test_rad2decimal_deg()
   test_rad2grad()
   test_grad2rad()
   test_grad2deg()
   test_deg2grad()
   test_dms_deg2decimal_deg()
